...
//	head.addView(TextView("这里是消息中心"));
	...
	PopMessage.setHeight(500);
	PopMessage.setWidth(800);
...
function RemoveMessage(v){try{
	var rootView = v.getParent().getParent().getParent();
	//-------------subsub--------sulayout----layout
	var subroot =  v.getParent().getParent();
	rootView.removeView(subroot);
	for(var i = 0; i < MessageViews.length; i++)
	{
		if(MessageViews[i][0] == rootView)
...
function TaskCenter() {
		...
		var check = Button("查看", {onClick : function(v) {try
								   {
									   ...
									   pathm = "sdcard/Tasks/" + pathm.replaceAll("^任务名称:", "") + "/icon.png"; 
	...
	PopTasks.setHeight(300);
	PopTasks.setWidth(400);
...
function Settings() {
	...
	layout.addView(Button("保存设置", {onClick : function(v) {try
								  {
									  var out = new java.io.FileOutputStream("/sdcard/Tasks/Settings.ini");
...
function ReadSettings() {try
	{
		var tmp = [];
		var ini_file = new java.io.File("/sdcard/Tasks/Settings.ini");
...
function IniRegexp(string) {
	...
	var regexps = 
	[
		[/^background_music[\s]:[\w\W]*/i, "background_music"],
		[/^default_font[\s]:[\w\W]*/i, "default_font"]
	];
	for (var i = 0; i < regexps.length; i++)
	{
		if (regexps[i][0].test(string))
		{
			...
			{
				case regexps[0][1] :
					result = string.replaceAll("^background_music[\\s]:", "");
					break;
				case regexps[1][1] :
					result = string.replaceAll("^default_font[\\s]:", "");
					break;
			}
		}
	}
...
function SubMenu(Width, Height, ContentView, Location, X, Y, Focusable, isShow) {
	...
	pop.setWidth(Width == null ? 100 : Width);
	pop.setHeight(Height == null ? 100 : Height);
...
function getInputStream(input) {
	...
	return new java.io.FileInputStream("/storage/sdcard0/Tasks/Task-Resource/" + input);
}
...
function debug() {
	...
	Popdebug.setWidth(450); 
	Popdebug.setHeight(350);
...
function entityRemoved(Entity) {try
	{
		eval(onEntityRemoved);
		client_bug("EntityRemove-Hook:" + "实体:" + Entity);
	}catch(error){Run({run:function() {Alert(error);}});}}
...
function attackHook(Player/*攻击方*/, Entity) {try
	{
		eval(onAttackHook);
		client_bug("Attack-Hook:" + "攻击方:" + Player + " " + "实体:" + Entity);
	}catch(error){Run({run:function() {Alert(error);}});}}
function entityAddedHook(Entity) {try
	{
		eval(onEntityAddedHook);
		client_bug("EntityAdd-Hook:" + "实体:" + Entity);
	}catch(error){Run({run:function() {Alert(error);}});}}
function deathHook(Player/*凶手*/, Entity) {try
	{
		eval(onDeathHook);
		client_bug("Deat-Hook:" + "凶手:" + Player + " " + "实体:" + Entity);
	}catch(error){Run({run:function() {Alert(error);}});}}
...
function explodeHook(Entity/*爆炸的实体*/, X, Y, Z, Power/*威力*/, O) {try
	{
		eval(onExplodeHook);
		client_bug("Explode-Hook:" + "Entity:" + Entity + " X:" + X + " Y:" + Y + " Z" + Z);
	}catch(error){Run({run:function() {Alert(error);}});}}
function levelEventHook(Entity/*动作的实体*/, Event/*事件类型*/, X, Y, Z, Data) {try
	{
...
function WriteTasksStringFile(SubFile, Content, isOverride) {try
	{
		...
		output.write(new java.lang.String(Content), isOverride == true);
		output.close();
	}catch(err){Run({run : function() {Alert(err + "\nline:" + err.lineNumber);}});}}
...
function WriteTasksDataFile(SubFile, Data, isOverride) {try
	{
		var output = new java.io.FileOutputStream(Sdcard_Tasks + SubFile);
		output.write(Data, isOverride == true);
		output.close();
	}catch(err){Run({run : function() {Alert(err + "\nline:" + err.lineNumber);}});}}
/******************************************************************************************************************************************************/
//提示函数
